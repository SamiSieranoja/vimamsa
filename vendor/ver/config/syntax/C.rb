...
{ fileTypes: ["c", "h"],
  ...
  keyEquivalent: "^~C",
  name: "C",
  patterns: [{ include: "#preprocessor-rule-enabled" },
             { include: "#preprocessor-rule-disabled" },
             { include: "#preprocessor-rule-other" },
             { include: "#comments" },
             { match: /\b(?<_1>break|case|continue|default|do|else|for|goto|if|_Pragma|return|switch|while)\b/,
               name: "keyword.control.c" },
             { match: /\b(?<_1>asm|__asm__|auto|bool|_Bool|char|_Complex|double|enum|float|_Imaginary|int|long|short|signed|struct|typedef|union|unsigned|void)\b/,
               name: "storage.type.c" },
             ...
             { include: "#sizeof" },
             { match: /\b(?<_1>(?<_2>0(?<_3>x|X)[0-9a-fA-F]*)|(?<_4>(?<_5>[0-9]+\.?[0-9]*)|(?<_6>\.[0-9]+))(?<_7>(?<_8>e|E)(?<_9>\+|-)?[0-9]+)?)(?<_10>L|l|UL|ul|u|U|F|f|ll|LL|ull|ULL)?\b/,
        	...
        	/,
              beginCaptures: { 1 => { name: "keyword.control.import.define.c" },
             ...
             { include: "#pragma-mark" },
             { begin: /^\s*#\s*(?<_1>define|defined|elif|else|if|ifdef|ifndef|line|pragma|undef)\b/,
              captures: { 1 => { name: "keyword.control.import.c" } },
              end: "(?=(?://|/\\*))|$",
              name: "meta.preprocessor.c",
              patterns: [{ match: /(?>\\\s*\n)/, name: "punctuation.separator.continuation.c" }] },
             { match: /\b(?<_1>u_char|u_short|u_int|u_long|ushort|uint|u_quad_t|quad_t|qaddr_t|caddr_t|daddr_t|dev_t|fixpt_t|blkcnt_t|blksize_t|gid_t|in_addr_t|in_port_t|ino_t|key_t|mode_t|nlink_t|id_t|pid_t|off_t|segsz_t|swblk_t|uid_t|id_t|clock_t|size_t|ssize_t|time_t|useconds_t|suseconds_t)\b/,
               name: "support.type.sys-types.c" },
             { match: /\b(?<_1>pthread_attr_t|pthread_cond_t|pthread_condattr_t|pthread_mutex_t|pthread_mutexattr_t|pthread_once_t|pthread_rwlock_t|pthread_rwlockattr_t|pthread_t|pthread_key_t)\b/,
               name: "support.type.pthread.c" },
             { match: /\b(?<_1>int8_t|int16_t|int32_t|int64_t|uint8_t|uint16_t|uint32_t|uint64_t|int_least8_t|int_least16_t|int_least32_t|int_least64_t|uint_least8_t|uint_least16_t|uint_least32_t|uint_least64_t|int_fast8_t|int_fast16_t|int_fast32_t|int_fast64_t|uint_fast8_t|uint_fast16_t|uint_fast32_t|uint_fast64_t|intptr_t|uintptr_t|intmax_t|intmax_t|uintmax_t|uintmax_t)\b/,
               name: "support.type.stdint.c" },
             { match: /\b(?<_1>noErr|kNilOptions|kInvalidID|kVariableLengthArray)\b/,
               name: "support.constant.mac-classic.c" },
             { match: /\b(?<_1>AbsoluteTime|Boolean|Byte|ByteCount|ByteOffset|BytePtr|CompTimeValue|ConstLogicalAddress|ConstStrFileNameParam|ConstStringPtr|Duration|Fixed|FixedPtr|Float32|Float32Point|Float64|Float80|Float96|FourCharCode|Fract|FractPtr|Handle|ItemCount|LogicalAddress|OptionBits|OSErr|OSStatus|OSType|OSTypePtr|PhysicalAddress|ProcessSerialNumber|ProcessSerialNumberPtr|ProcHandle|Ptr|ResType|ResTypePtr|ShortFixed|ShortFixedPtr|SignedByte|SInt16|SInt32|SInt64|SInt8|Size|StrFileName|StringHandle|StringPtr|TimeBase|TimeRecord|TimeScale|TimeValue|TimeValue64|UInt16|UInt32|UInt64|UInt8|UniChar|UniCharCount|UniCharCountPtr|UniCharPtr|UnicodeScalarValue|UniversalProcHandle|UniversalProcPtr|UnsignedFixed|UnsignedFixedPtr|UnsignedWide|UTF16Char|UTF32Char|UTF8Char)\b/,
    		...
    		(?<_3>
    		 ...
    		 \s*(?=\()/,
              beginCaptures: { 1 => { name: "punctuation.whitespace.function.leading.c" },
              ...
              patterns: [{ include: "#comments" },
                         ...
                         { include: "#block" }] }],
  repository: { access: { match: /\.[a-zA-Z_][a-zA-Z_0-9]*\b(?!\s*\()/,
                         name: "variable.other.dot-access.c" },
               block: { begin: /\{/,
                       end: "\\}",
                       name: "meta.block.c",
                       patterns: [{ include: "#block_innards" }] },
               block_innards: { patterns: [{ include: "#preprocessor-rule-enabled-block" },
	...
	(?: (?<=operator) (?: [-*&<>=+!]+ | \(\) | \[\] ) )?  # if it is a C++ operator
	 ...
	 \s*(?<_2>\()/,
                                          ...
                                          { include: "#block" },
                                          { include: "$base" }] },
               comments: { patterns: [{ captures: { 1 => { name: "meta.toc-list.banner.block.c" } },
                                       match: /^\/\* =(?<_1>\s*.*?)\s*= \*\/$\n?/,
                                     ...
                                     { begin: /\/\//,
                                      ...
                                      patterns: [{ match: /(?>\\\s*\n)/,
                                                   name: "punctuation.separator.continuation.c++" }] }] },
               disabled: { begin: /^\s*#\s*if(?<_1>n?def)?\b.*$/,
               ...
               :"preprocessor-rule-disabled" => { begin: /^\s*(?<_1>#(?<_2>if)\s+(?<_3>0)\b).*/,
                                                 ...
                                                 patterns: [{ begin: /^\s*(?<_1>#\s*(?<_2>else)\b)/,
                                                            ...
                                                            { begin: //,
                                                             ...
                                                             patterns: [{ include: "#disabled" }, { include: "#pragma-mark" }] }] },
               :"preprocessor-rule-disabled-block" => { begin: /^\s*(?<_1>#(?<_2>if)\s+(?<_3>0)\b).*/,
                                                       captures: { 1 => { name: "meta.preprocessor.c" },
                                                                   2 => { name: "keyword.control.import.if.c" },
                                                                   3 => { name: "constant.numeric.preprocessor.c" } },
                                                       end: "^\\s*(#\\s*(endif)\\b)",
                                                       patterns: [{ begin: /^\s*(?<_1>#\s*(?<_2>else)\b)/,
                                                                  ...
                                                                  { begin: //,
                                                                   ...
                                                                   patterns: [{ include: "#disabled" }, { include: "#pragma-mark" }] }] },
               :"preprocessor-rule-enabled" => { begin: /^\s*(?<_1>#(?<_2>if)\s+(?<_3>0*1)\b)/,
                                                captures: { 1 => { name: "meta.preprocessor.c" },
                                                            2 => { name: "keyword.control.import.if.c" },
                                                            3 => { name: "constant.numeric.preprocessor.c" } },
                                                end: "^\\s*(#\\s*(endif)\\b)",
                                                patterns: [{ begin: /^\s*(?<_1>#\s*(?<_2>else)\b).*/,
                                                           ...
                                                           { begin: //,
                                                            ...
                                                            patterns: [{ include: "$base" }] }] },
               :"preprocessor-rule-enabled-block" => { begin: /^\s*(?<_1>#(?<_2>if)\s+(?<_3>0*1)\b)/,
                                                      captures: { 1 => { name: "meta.preprocessor.c" },
                                                                  2 => { name: "keyword.control.import.if.c" },
                                                                  3 => { name: "constant.numeric.preprocessor.c" } },
                                                      end: "^\\s*(#\\s*(endif)\\b)",
                                                      patterns: [{ begin: /^\s*(?<_1>#\s*(?<_2>else)\b).*/,
                                                                 ...
                                                                 { begin: //,
                                                                  ...
                                                                  patterns: [{ include: "#block_innards" }] }] },
               :"preprocessor-rule-other" => { begin: /^\s*(?<_1>#\s*(?<_2>if(?<_3>n?def)?)\b.*?(?:(?=(?:\/\/|\/\*))|$))/,
                                              captures: { 1 => { name: "meta.preprocessor.c" },
                                                          2 => { name: "keyword.control.import.c" } },
                                              end: "^\\s*(#\\s*(endif)\\b).*$",
                                              patterns: [{ include: "$base" }] },
               :"preprocessor-rule-other-block" => { begin: /^\s*(?<_1>#\s*(?<_2>if(?<_3>n?def)?)\b.*?(?:(?=(?:\/\/|\/\*))|$))/,
                                                    captures: { 1 => { name: "meta.preprocessor.c" },
                                                                2 => { name: "keyword.control.import.c" } },
                                                    end: "^\\s*(#\\s*(endif)\\b).*$",
                                                    patterns: [{ include: "#block_innards" }] },
               sizeof: { match: /\b(?<_1>sizeof)\b/, name: "keyword.operator.sizeof.c" },
               string_escaped_char: { patterns: [{ match: /\\(?<_1>\\|[abefnprtv'"?]|[0-3]\d{,2}|[4-7]\d?|x[a-fA-F0-9]{,2})/,
                                                  name: "constant.character.escape.c" },
                                                { match: /\\./, name: "invalid.illegal.unknown-escape.c" }] },
               string_placeholder: { patterns: [{ match: /(?x)%
