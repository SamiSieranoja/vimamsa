...
{ comment: "\n\tTODO: unresolved issues\n\n\ttext:\n\t\"p << end\n\tprint me!\n\tend\"\n\tsymptoms:\n\tnot recognized as a heredoc\n\tsolution:\n\tthere is no way to distinguish perfectly between the << operator and the start\n\tof a heredoc. Currently, we require assignment to recognize a heredoc. More\n\trefinement is possible.\n\t• Heredocs with indented terminators (<<-) are always distinguishable, however.\n\t• Nested heredocs are not really supportable at present\n\n\ttext:\n\tprint <<-'THERE' \n\tThis is single quoted. \n\tThe above used \#{Time.now} \n\tTHERE \n\tsymtoms:\n\tFrom Programming Ruby p306; should be a non-interpolated heredoc.\n\t\n\ttext:\n\t\"a\\332a\"\n\tsymptoms:\n\t'\\332' is not recognized as slash3.. which should be octal 332.\n\tsolution:\n\tplain regexp.. should be easy.\n\n    text:\n    val?(a):p(b)\n    val?'a':'b'\n    symptoms:\n    ':p' is recognized as a symbol.. its 2 things ':' and 'p'.\n    :'b' has same problem.\n    solution:\n    ternary operator rule, precedence stuff, symbol rule.\n    but also consider 'a.b?(:c)' ??\n",
  ...
  keyEquivalent: "^~R",
  name: "Ruby",
  patterns: [{ captures: { 1 => { name: "keyword.control.class.ruby" },
               ...
               match: /^\s*(?<_1>class)\s+(?<_2>(?<_3>[.a-zA-Z0-9_:]+(?<_4>\s*(?<_5><)\s*[.a-zA-Z0-9_:]+)?)|(?<_6>(?<_7><<)\s*[.a-zA-Z0-9_:]+))/,
               name: "meta.class.ruby" },
             { captures: { 1 => { name: "keyword.control.module.ruby" },
               ...
               match: /^\s*(?<_1>module)\s+(?<_2>(?<_3>[A-Z]\w*(?<_4>::))?(?<_5>[A-Z]\w*(?<_6>::))?(?<_7>[A-Z]\w*(?<_8>::))*[A-Z]\w*)/,
             ...
             { comment: "everything being a reserved word, not a value and needing a 'end' is a..",
               match: /(?<!\.)\b(?<_1>BEGIN|begin|case|class|else|elsif|END|end|ensure|for|if|in|module|rescue|then|unless|until|when|while)\b(?![?!])/,
             ...
             { comment: " just as above but being not a logical operation",
               match: /(?<!\.)\b(?<_1>alias|alias_method|break|next|redo|retry|return|super|undef|yield)\b(?![?!])|\bdefined\?|\bblock_given\?/,
             ...
             { comment: " everything being a method but having a special function is a..",
               match: /\b(?<_1>initialize|new|loop|include|extend|raise|attr_reader|attr_writer|attr_accessor|attr|catch|throw|private|module_function|public|protected)\b(?![?!])/,
	        ...
	        /,
              beginCaptures: { 1 => { name: "keyword.control.def.ruby" },
	        ...
	        /,
              ...
              name: "meta.function.method.with-arguments.ruby",
              patterns: [{ include: "$self" }] },
             { captures: { 1 => { name: "keyword.control.def.ruby" },
	        ...
	        /,
             ...
             { match: /\b(?<_1>0[xX]\h(?>_?\h)*|\d(?>_?\d)*(?<_2>\.(?![^[:space:][:digit:]])(?>_?\d)*)?(?<_3>[eE][-+]?\d(?>_?\d)*)?|0[bB][01]+)\b/,
	   ...
	   \s*(?<_1>(?<_2>\/))(?![*+{}?])
	/,
              captures: { 1 => { name: "string.regexp.classic.ruby" },
             ...
             { begin: /%[qws](?<_1>[^\w])/,
              ...
              endCaptures: { 0 => { name: "punctuation.definition.string.end.ruby" } },
              name: "string.quoted.other.literal.lower.ruby",
              patterns: [{ comment: "Cant be named because its not neccesarily an escape.",
                          match: /\\./ }] },
             { captures: { 1 => { name: "punctuation.definition.constant.ruby" } },
               comment: "symbols",
               match: /(?<!:)(?<_1>:)(?>[a-zA-Z_]\w*(?>[?!]|=(?![>=]))?|===?|>[>=]?|<[<=]?|<=>|[%&`\/\|]|\*\*?|=?~|[-+]@?|\[\]=?|@@?[a-zA-Z_]\w*)/,
             ...
             { comment: "\n\t\t\tmatches questionmark-letters.\n\n\t\t\texamples (1st alternation = hex):\n\t\t\t?\\x1     ?\\x61\n\n\t\t\texamples (2nd alternation = octal):\n\t\t\t?\\0      ?\\07     ?\\017\n\n\t\t\texamples (3rd alternation = escaped):\n\t\t\t?\\n      ?\\b\n\n\t\t\texamples (4th alternation = meta-ctrl):\n\t\t\t?\\C-a    ?\\M-a    ?\\C-\\M-\\C-\\M-a\n\n\t\t\texamples (4th alternation = normal):\n\t\t\t?a       ?A       ?0 \n\t\t\t?*       ?\"       ?( \n\t\t\t?.       ?#\n\t\t\t\n\t\t\t\n\t\t\tthe negative lookbehind prevents against matching\n\t\t\tp(42.tainted?)\n\t\t\t",
               match: /(?<!\w)\?(?<_1>\\(?<_2>x\h{1,2}(?!\h)\b|0[0-7]{0,2}(?![0-7])\b|[^x0MC])|(?<_3>\\[MC]-)+\w|[^\s\\])/,
               name: "constant.numeric.ruby" },
             { begin: /^__END__\n/,
              ...
              patterns: [{ begin: /(?=<?xml|<(?i:html\b)|!DOCTYPE (?i:html\b))/,
                          end: "(?=not)impossible",
             ...
             { begin: /(?<=\{|do|\{\s|do\s)(?<_1>\|)/,
              ...
              patterns: [{ match: /[_a-zA-Z][_a-zA-Z0-9]*/, name: "variable.other.block.ruby" },
  ...
  repository: { escaped_char: { match: /\\(?:[0-7]{1,3}|x[\da-fA-F]{1,2}|.)/,
                               name: "constant.character.escape.ruby" },
               heredoc: { begin: /^<<-?\w+/, end: "$", patterns: [{ include: "$self" }] },
               interpolated_ruby: { patterns: [{ captures: { 0 => { name: "punctuation.section.embedded.ruby" },
               ...
               nest_brackets_i: { begin: /\[/,
                                 captures: { 0 => { name: "punctuation.section.scope.ruby" } },
                                 end: "\\]",
                                 patterns: [{ include: "#interpolated_ruby" },
                                            { include: "#escaped_char" },
                                            { include: "#nest_brackets_i" }] },
               nest_brackets_r: { begin: /\[/,
                                 captures: { 0 => { name: "punctuation.section.scope.ruby" } },
                                 end: "\\]",
                                 patterns: [{ include: "#regex_sub" }, { include: "#nest_brackets_r" }] },
               nest_curly: { begin: /\{/,
               ...
               nest_curly_and_self: { patterns: [{ begin: /\{/,
                                                 captures: { 0 => { name: "punctuation.section.scope.ruby" } },
                                                 end: "\\}",
               ...
               nest_parens_r: { begin: /\(/,
                               ...
                               patterns: [{ include: "#regex_sub" }, { include: "#nest_parens_r" }] },
               regex_sub: { patterns: [{ include: "#interpolated_ruby" },
                                      { include: "#escaped_char" },
                                      { captures: { 1 => { name: "punctuation.definition.arbitrary-repitition.ruby" },
