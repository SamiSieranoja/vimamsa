...
def page_down
  # $view.signal_emit("search-changed")
  # GTK_MOVEMENT_PAGES

  # Ripl.start :binding => binding
  #$view.get_cursor_locations
  # $view.signal_emit("move-cursor", Gtk::MovementStep.new(:PAGES))
  $view.signal_emit("move-cursor", Gtk::MovementStep.new(:PAGES), 1, false)
  # $view.signal_emit("move-cursor", Gtk::MovementStep.new(:WORDS),1,false)
  # $view.signal_emit("move-cursor", Gtk::MovementStep.new(:DISPLAY_LINES),1,false)
  # $view.signal_emit("move-cursor", Gtk::MovementStep.new(:DISPLAY_LINE_ENDS),1,false)

  # curpos = $view.buffer.cursor_position
  # buf.set_pos(curpos)
  ...
  if false
    ...
    buf.set_pos(curpos)
    b = $view.buffer
    iter = b.get_iter_at(:offset => curpos)
    within_margin = 0.1 #margin as a [0.0,0.5) fraction of screen size
    use_align = true
...
class VMAg
  ...
  def test_edit
    ...
    # [5, 3001, 1, "o"]
    # [5, 3001, 1, "i"]
    # [10, 2, 13, "come together"]

    5.times { buf.move(FORWARD_CHAR) }
    2.times { buf.delete(CURRENT_CHAR_FORWARD) }
    5.times { buf.move(FORWARD_CHAR) }
    buf.insert_txt("come together")
    puts buf.to_s[0..150]
    # for d in buf.deltas
    ...
    puts "========================"
    puts @buf1.text[0..150]
    puts "========================"
    puts buf[0..150]
    puts "========================"
    # puts buf.deltas.inspect
  end
  ...
  def init_window
    ...
    # fname = expand_if_existing(ARGV[0])
    # puts "fname=#{fname}"
    # if fname.nil?
    # fname = "test.txt"
    # end
