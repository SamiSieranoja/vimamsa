...
def cyberpunk_css
  ...
  # Mode badge colors match the baseline theme in gui.rb; here each gets a
  # neon glow of its own color.
  badge_glow = { "mode-command" => "#7c68f2", "mode-insert" => "#78bf78",
    ...
    .map { |cls, col| "label.mode-badge.#{cls} { box-shadow: 0 0 12px #{cyber_rgba(col, 0.55)}; }" }
    .join("\n  ")
