...
def gui_select_buffer()
    ...
    qt_select_window(buffer_list,$select_keys.collect{|x| x.upcase},
                     method(:gui_select_buffer_callback))
end
...
def gui_select_buffer_init()
    $at.add_mode('S')
    bindkey 'S enter', '$at.set_mode(COMMAND)'
    bindkey 'S return', '$at.set_mode(COMMAND)'
